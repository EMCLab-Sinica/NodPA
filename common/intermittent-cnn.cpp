...
static void handle_node(Model *model, uint16_t node_idx, PropagationDirection direction) {
    ...
    if (direction == PropagationDirection::FORWARD) {
        ...
        backward_handlers[cur_node->op_type](model, input, output, cur_node);
    }
...
static void run_model(int8_t *ansptr, const ParameterInfo **output_node_ptr) {
    ...
    // topological sort guarantees that the last node in an ONNX model is always the output node
    ...
    if (sample_idx == 0) {
        for (uint8_t buffer_idx = 0, ofm_idx = 0; buffer_idx < buffer_len; buffer_idx++) {
...
#endif
            {
                ...
                MY_ASSERT((got_real == expected) /* both are zero */ || (error <= 0.15),
